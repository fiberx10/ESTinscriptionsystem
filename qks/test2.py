...
# 
# 
# for the db make sure to chnage those things : 
# 
# 
# personne table  :  "e-mail "  => "e-mail" /// remove space after the word
# inscription_diplome table  :  "numero_inscription" [CHAR  : 10]  => "numero_inscription" [CHAR  : 20] /// lenght 10 to lenght 20
# personne table  :  "situation familail" [CHAR  : 1]  => "situaltion_familail" [CHAR  : 100] /// lenght 1 to lenght 100
# 
# "
...
class getAllMatch(Dbconnect):
    ...
    def get_admission_search(self, data: list):
        ...
        join_query = " diplome INNER JOIN filiere ON diplome.id=diplome INNER JOIN offre_formation ON filiere.id=filiere INNER JOIN admission ON offre_formation.id=offre_formation "
...
def create_pdf(full_name, cne, cin, filiere, numero_inscrit, annee):
    ...
    can.drawString(180, 537, numero_inscrit)
...
class WelcomScreen(QDialog, FORM_CLASS):
    ...
    def potmessage(self):
        QMessageBox.information(self, "About",
                                "EST fes Apps Inscription version 0.1")
...
class admis_Screen(QDialog, FORM_CLASS, getAllMatch):
    ...
    def gotoCHek(self):
        try:
            ...
            # chouichou work
            # data darori tkoun type string kolha
            self.dataEtudeGlobal = {
                "cne": 'cne',
                "cin": 'cin',
                "nom": 'yahya',
                "prenom": 'rhiba',
                "nom_ar": 'yahya',
                "prenom_ar": 'rhiba',
                "naissance_date": "17/12/2001",  # darori haka o maykoun in variable hta space
                "naissance_lieu": 'rabat',
                "naissance_province": 'agdal',
                "sexe": 'H',  # or "F" or ''
                "naissance_pays": 'maroc',
                "nationalite": 'Maroc',
                "e-mail": 'yahyarhiba@gmail.com',
                "telephone": '0671769782',
                "bac_annee": "2018",
                "bac_mention": "A Bien",
                "bac_province": "provinceBac",
                "bac_serie": "ste",
                "bac_type": 'Marocain',  # or "Mission" or "Etranger" or "" wa7da men hado darori
            }
            print(self.DAta_Etude)
    ...
    def getinfo(self):
        ...
        self.bdit = []
        if Annee != '':
            self.bdit.append(['annee_universitere', Annee])
        else:
            self.bdit.append([])
        if Phase != "-----------":
    ...
    def getinfoTable(self):
        self.tableInfo = []
        self.tableInfo = [('S138216469', 'chouichou', decimal.Decimal('14.500'), 'mohamed', None, 'GE'),
                          ('S138216479', 'rhiba', decimal.Decimal('15.500'), 'yahya', None, 'GLT')]
        # get data search
...
FORM_CLASS, _ = loadUiType(
    path.join(path.dirname(__file__), 'z_ficheEtude1.ui'))
...
class fich6(QDialog, FORM_CLASS):
    ...
    def rempliInfo(self):
        data_schema = {}
        # self.cNe.setText(data_schema["cne"])
        # self.fname.setText(data_schema["nom"])
        # self.lname.setText(data_schema["prenom"])
        # self.NmrCin.setText(data_schema["cin"])
        # self.nomArabic.setText(data_schema["nom_ar"])
        # self.prenoArab.setText(data_schema["prenom_ar"])
    ...
    def check(self):
        ...
        self.Retour_1.clicked.connect(self.gotoAdmission)
...
class fich9(QDialog, FORM_CLASS, getAllMatch):
    ...
    def chek(self):
        ...
        self.Valider.clicked.connect(self.gotovalide)
        self.Annuler.clicked.connect(self.annuler)
        self.Imprimer.clicked.connect(self.chekOkya)
    ...
    def okayNumber(self, num):
        if self.test(num) == False:
            while self.test(num) == False:
                num = self.randNuberIscri()
            return num
    ...
    def get(self):
        numb = self.randNuberIscri()
        if self.test(numb) == False:
            numb = self.okayNumber(numb)
        return numb
...
class fich1(QDialog, FORM_CLASS):
    ...
    def check(self):
        ...
        self.file_name = ''
        self.Import.clicked.connect(self.getImage)
        self.Retour_1.clicked.connect(self.gotoWelcom)
    ...
    def get_info(self):
        # hena kanaghedo ga3 lma3lomat li 3amr user men ba3ed ma clika
        # 3la suivant
        # self.CNe = self.cnE.text()
        # self.NOm = self.name.text()
        # self.PRENo = self.Preno.text()
        # self.NmrCin = self.Nmr_Cin.text()
        # self.Nom_arabe = self.nom_arabe.text()
        # self.Preno_arabe = self.preno_arab.text()
        # self.dateNais = self.dateNaiss.text()
        # self.lieuDeNaiss = self.lieuDeNais.text()
        # self.lieuDeNais_arabe = self.lieuDeNais_arab.text()
        # self.ProvenceDeNaiss = self.Provence.currentText()
        # self.payDeNaiss = self.Pays_1.currentText()
        # self.TypHandicap = self.Handicap.currentText()
        # self.nationality = self.Pays_2.currentText()
        # self.categoriSocioProf = self.csp.currentText()
        # self.sexe = self.Sexe()
        # self.situationFamily = self.stiuall()
        # self.Picturpath = self.file_name
        # hena kteb charet dyalk ya3ni chno khaso ykoun 3mer darori 3ade
        # te9der douz fich 2 db 7na f l fich 1
        """
        KolchiMzyan = self.tester()
        if KolchiMzyan :
            go to rempli fiche 2
        else :
            print eror il faut completer les cham important !!! 
        """
        self.remplifich234()
        # aprés go to fich 2
        self.gotofiche2()
